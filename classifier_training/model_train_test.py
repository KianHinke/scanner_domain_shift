...
def dataloading(scanner, annotation_file, data_folder, test=False):
    ...
    if test == True:
        ...
        test_data = torch.load(os.path.join(data_folder, f'{scanner}_test.pt'))
        ...
        train_data = torch.load(os.path.join(data_folder, f'{scanner}_train.pt'))
        valid_data = torch.load(os.path.join(data_folder, f'{scanner}_valid.pt'))
        test_data = torch.load(os.path.join(data_folder, f'{scanner}_test.pt'))
    ...
    if test == True:
        test_loader = generate_testloader_only( test_files, batch_size, reverse_dict)
        return test_loader    
    else:
        train_loader, valid_loader, test_loader = generate_dataloaders(train_files, valid_files, test_files, batch_size, reverse_dict)
        return train_loader, valid_loader, test_loader
    # print("First batch from Training DataLoader:")
    # x_batch, y_batch, slide_ids = next(iter(train_loader))
    # print(f"Unique labels in y_batch: {np.unique(y_batch)}")

    

def train(train_loader, valid_loader, modelname, modelsavepath, scanner, lr, epochs, input_size, hidden_size, hidden_size1, hidden_size2, fscores):
    ...
    if all(f1 > score for score in fscores):
        ...
        if modelname == "ResNet18" or modelname == "ResNet50" or modelname == "ResNet101":
            savepath = os.path.join(modelsavepath, f'{scanner}_{modelname}_epoch_{epoch+1}_lr_{lr}.pth')
        elif modelname == "TwoLayerNN": 
            savepath = os.path.join(modelsavepath, f'{scanner}_{modelname}_epoch_{epoch+1}_lr_{lr}_{hidden_size}.pth')
        elif modelname == "ThreeLayerNN":
            savepath = os.path.join(modelsavepath, f'{scanner}_{modelname}_epoch_{epoch+1}_lr_{lr}_{hidden_size1}_{hidden_size2}.pth')
...
def test(test_loader, model, modelname, result_file, train_accuracies=[], valid_accuracies=[], modelpath = ""):
    print("------------------------------------")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
if __name__ == "__main__":
    ...
    assert 'data_folder' in config, "Data folder not found in config"
    data_folder = config['data_folder']
    assert 'results_file' in config, "Result file not found in config"
    ...
    assert 'models' in config or 'modelpaths' in config or 'models_folder_path' in config, "No models or model paths found in config"
    ...
    if 'models_folder_path' in config:        
        ...
        for path in all_pth_file_paths:
                ...
                #check if the scanner of the model to be trained has changed (i.e. if new data has to be loaded)
                if scanner != scanner_before:
                    print(f"Loading data for testing {scanner}")  
                    test_loader =  dataloading(scanner, annotation_file, data_folder, test=True)
    ...
    if 'models' in config:
        ...
        for scanner in scanners:
            ...
            train_loader, valid_loader, test_loader = dataloading(scanner, annotation_file, data_folder, test=False)
            ...
            for modelname in config['models']:
                ...
                for epochs in config['epochs']:
                    ...
                    for lr in config['lrs']:
                        ...
                        # Train model and save to modelsavepath
                        if not os.path.exists(modelsavepath):
                            os.makedirs(modelsavepath)
                        model, train_accuracies, valid_accuracies, f1score = train(train_loader, valid_loader, modelname, modelsavepath, scanner, lr, epochs, input_size, hidden_size, hidden_size1, hidden_size2, fscores)
